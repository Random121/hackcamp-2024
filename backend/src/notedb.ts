...
db.serialize(() => {
    ...
    db.run('CREATE TABLE notes (id INTEGER primary_key, title text, note text, location point) ')
});

export function addNotetoDatabase(title, note, latitude, longitude) {
    return new Promise<void>((resolve, reject) => {
        const insert = db.prepare("INSERT INTO notes(title, note, location) VALUES (?, ?, GeomFromText(?, 4326))");
